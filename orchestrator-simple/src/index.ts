...
app.post("/start", async (req, res) => {
    ...
    try {
        const kubeManifests = readAndParseKubeYaml(path.join(process.cwd(), "service.yaml"), replId);
        for (const manifest of kubeManifests) {
            try {
                ...
                if ((err.body && typeof err.body === 'string' && err.body.includes('AlreadyExists')) || err.statusCode === 409 || err.code === 409) {
                    console.log(`Resource ${manifest.kind} already exists. Skipping.`);
                } else {
                    throw err;
                }
            }
